...
import { Follow, Post, PostComment, ProfilePatch, Story, User } from '../types';
...
const Profile: React.FC<ProfileProps> = ({
  ...
  const friends = useMemo(
    () => friendIds.map((id) => usersById[id]).filter((item): item is User => Boolean(item)),
    [friendIds, usersById]
  );
  ...
  const submit = (event: React.FormEvent) => {
    ...
    onSave({ displayName, bio, status, avatar, coverImage });
    setEditing(false);
  };
    ...
    <div className="flex flex-col pb-20">
      ...
      <div className="px-4 md:px-6 relative mb-6">
        <div className="flex justify-between items-end -mt-12 md:-mt-16 mb-4">
          <div className="p-1 bg-white dark:bg-black rounded-[32px] relative">
            <img src={userAvatar(profileUser)} className="w-24 h-24 md:w-32 md:h-32 rounded-[28px] object-cover border-4 border-white dark:border-black" alt="Avatar" />
            <span
          ...
          <div className="flex gap-2">
            {isOwn ? (
              <button onClick={() => setEditing((prev) => !prev)} className="border-2 border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-900 px-4 py-2 rounded-2xl font-bold text-slate-900 dark:text-white transition-colors">
                {editing ? 'Cancel' : 'Edit Profile'}
              </button>
            ) : (
              <>
                <button onClick={() => onMessage(profileUser.id)} className="border-2 border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-900 px-4 py-2 rounded-2xl font-bold text-slate-900 dark:text-white transition-colors">Message</button>
                <button onClick={() => onToggleFollow(profileUser.id)} className={`px-4 py-2 rounded-2xl font-bold transition-colors ${isFollowing ? 'border-2 border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-900 text-slate-900 dark:text-white' : 'bg-slate-900 dark:bg-white dark:text-black text-white hover:bg-slate-800 dark:hover:bg-slate-200'}`}>
            ...
            <button
              ...
              className="border-2 border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-900 px-4 py-2 rounded-2xl font-bold text-slate-900 dark:text-white transition-colors"
            >
              Copy link
            </button>
          </div>
        ...
        <div className="space-y-4">
          ...
          <div className="flex gap-6 text-sm text-slate-500 font-medium">
            ...
            <span className="flex items-center gap-1.5"><b className="text-slate-900 dark:text-white">{profileUser.role}</b></span>
          </div>
        </div>
      ...
      <div className="flex border-b border-slate-100 dark:border-slate-800 px-2 md:px-6 overflow-x-auto">
        {[
          ...
          <button
            ...
            className={`flex-1 py-4 text-sm font-bold transition-colors border-b-2 whitespace-nowrap min-w-[90px] ${
              activeTab === tab.id
