  ...
  return users.map((user) =>
    user.username === '313'
      ? {
          ...
          hiddenFromFriends: user.hiddenFromFriends ?? false,
...
const normalizePostReposts = (posts: Post[]): Post[] => {
  ...
  const deduped = ordered.filter((post) => {
    const rootPostId = post.repostOfPostId;
    const rootGroupPostId = post.repostOfGroupPostId;
    if (!rootPostId && !rootGroupPostId) return true;
    const key = rootPostId
      ? `post:${post.authorId}:${rootPostId}`
      : `group:${post.authorId}:${rootGroupPostId}`;
...
const normalizeDb = (input: Partial<SocialDb>): SocialDb => {
  ...
  const posts = normalizePostReposts(
    (input.posts ?? base.posts).map((post) => ({
      ...
      repostOfGroupPostId: post.repostOfGroupPostId,
      repostSourceGroupId: post.repostSourceGroupId,
    }))
  );
  ...
  return {
    ...
    posts,
    postComments: (input.postComments ?? base.postComments).map((comment) => ({
      ...comment,
    ...
    groupMembers,
    groupPosts,
    groupPostComments: (input.groupPostComments ?? base.groupPostComments).map((comment) => ({
      ...comment,
      likedBy: comment.likedBy ?? [],
    })),
